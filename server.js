...
const server = http.createServer(async (req, res) => {
  try {
    ...
    // יתרה עדכנית מהבנק
    // מחפש דווקא balanceType = expected
    if (u.pathname === "/api/account-balance") {
      ...
      function findExpected(value, depth = 0) {
        if (
          depth > 12 ||
          value === null ||
          value === undefined
      ...
      const balance = findExpected(data);

      if (balance === null) {
        return send(res, 404, {
          error: "Expected balance not found"
        });
      }
