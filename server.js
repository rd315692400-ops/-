...
function unwrapAccounts(data) {
  ...
  if (data?.result) return unwrapAccounts(data.result);
...
function extractAccountBalance(account) {
  ...
  for (const key of directKeys) {
    const n = numericBalance(account[key]);
  ...
  for (const type of preferredTypes) {
    const row = balances.find(
      b =>
        String(
          b?.balanceType || b?.type || ""
        ).toUpperCase() === type
    );

    const n = numericBalance(
      row?.balanceAmount ??
      row?.amount ??
      row?.value ??
      row?.balance
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    // מחזיר רק את היתרה, ללא פרטי חשבון
